...
import { AlienMaxGrowthInsight } from "@/lib/repositories/growthRepository";
import { BotIcon, ArrowUpRightIcon } from "@/components/icons";

export interface AlienMaxGrowthAdvisorWidgetProps {
  insights: AlienMaxGrowthInsight[];
}

export function AlienMaxGrowthAdvisorWidget({
  insights,
}: AlienMaxGrowthAdvisorWidgetProps) {
  return (
    <Card className="border-[#E4E4E7] bg-white space-y-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-bold text-[#111111] tracking-tight">
              Alien Max · Growth & Otimização
            </h3>
            <span className="text-[10px] text-[#71717A] block">
              Amostragem & Significância Estatística
            </span>
          </div>
        </div>
      ...
      <p className="text-xs text-[#52525B] leading-relaxed">
        O Alien Max apenas recomenda. <strong className="text-[#111111]">Nenhum teste A/B é validado ou descartado sem decisão da equipe.</strong>
      </p>
