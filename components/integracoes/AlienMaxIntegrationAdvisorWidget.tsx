...
import { AlienMaxIntegrationInsight } from "@/lib/repositories/integrationRepository";
import { BotIcon, ArrowUpRightIcon } from "@/components/icons";

export interface AlienMaxIntegrationAdvisorWidgetProps {
  insights: AlienMaxIntegrationInsight[];
}

export function AlienMaxIntegrationAdvisorWidget({
  insights,
}: AlienMaxIntegrationAdvisorWidgetProps) {
  return (
    <Card className="border-[#E4E4E7] bg-white space-y-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-bold text-[#111111] tracking-tight">
              Alien Max · Consultor de Infra & APIs
            </h3>
            <span className="text-[10px] text-[#71717A] block">
              Tokens, Permissões & Disparos de Pixels
            </span>
          </div>
        </div>
      ...
      <p className="text-xs text-[#52525B] leading-relaxed">
        O Alien Max apenas recomenda. <strong className="text-[#111111]">Nenhuma permissão OAuth ou token é revogado ou alterado automaticamente.</strong>
      </p>
