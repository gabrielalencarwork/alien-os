...
import { AlienMaxGoogleAdsInsight } from "@/lib/repositories/googleAdsRepository";
import { BotIcon, ArrowUpRightIcon } from "@/components/icons";

export interface AlienMaxGoogleAdsAdvisorWidgetProps {
  insights: AlienMaxGoogleAdsInsight[];
}

export function AlienMaxGoogleAdsAdvisorWidget({
  insights,
}: AlienMaxGoogleAdsAdvisorWidgetProps) {
  return (
    <Card className="border-[#E4E4E7] bg-white space-y-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-bold text-[#111111] tracking-tight">
              Alien Max · Otimização Google Ads
            </h3>
            <span className="text-[10px] text-[#71717A] block">
              Auditoria de Lances, ROAS & CTR
            </span>
          </div>
        </div>
      ...
      <p className="text-xs text-[#52525B] leading-relaxed">
        O Alien Max apenas recomenda. <strong className="text-[#111111]">Nenhum lance ou orçamento é alterado na API sem aprovação do gestor.</strong>
      </p>
