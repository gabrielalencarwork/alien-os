...
import { AlienMaxAgendaInsight } from "@/lib/repositories/meetingRepository";
import { BotIcon, ArrowUpRightIcon } from "@/components/icons";

export interface AlienMaxAgendaAdvisorWidgetProps {
  insights: AlienMaxAgendaInsight[];
}

export function AlienMaxAgendaAdvisorWidget({
  insights,
}: AlienMaxAgendaAdvisorWidgetProps) {
  return (
    <Card className="border-[#E4E4E7] bg-white space-y-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-bold text-[#111111] tracking-tight">
              Alien Max · Consultor Executivo
            </h3>
            <span className="text-[10px] text-[#71717A] block">
              Ocupação & Conflitos de Agenda
            </span>
          </div>
        </div>
      ...
      <p className="text-xs text-[#52525B] leading-relaxed">
        O Alien Max apenas recomenda. <strong className="text-[#111111]">Nenhuma reunião é reagendada ou cancelada sem confirmação.</strong>
      </p>
