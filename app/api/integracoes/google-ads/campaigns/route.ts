...
export async function POST(req: NextRequest) {
  try {
    ...
    const campaigns = await googleAdsConnector.listCampaigns(accessToken, customerId, developerToken);
    ...
    return NextResponse.json(
      { error: error?.message || "Erro ao consultar campanhas do Google Ads." },
      { status: 500 }
    );
  }
}
