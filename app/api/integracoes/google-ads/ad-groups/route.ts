...
export async function POST(req: NextRequest) {
  try {
    ...
    const adGroups = await googleAdsConnector.listAdGroups(accessToken, customerId, developerToken);
    ...
    return NextResponse.json(
      { error: error?.message || "Erro ao consultar grupos de anúncios do Google Ads." },
      { status: 500 }
    );
  }
}
